...
ecc.utils.getMenu = function (actions, grid, selected) {
    var menu = [];
...
ecc.utils.renderActions = function (value, props, row) {
    var res = [];
...
ecc.utils.formatDate = function (string) {
    if (string && string != '0000-00-00 00:00:00' && string != '-1-11-30 00:00:00' && string != 0) {
        ...
        return date.strftime(MODx.config.ecc_date_format);
    }
    else {
        return '&nbsp;';
    }
...
ecc.utils.renderBoolean = function (value, props, row) {
    return value
        ? String.format('<span class="green">{0}</span>', _('yes'))
        : String.format('<span class="red">{0}</span>', _('no'));
};

ecc.utils.renderReplace = function (value, replace, color) {
    if (!value) {
        return '';
    }
    else if (!replace) {
    ...
    return String.format('<span class="ecc-render-color" style="color: #{1}">{0}</span>', replace, color);
};

ecc.utils.renderMoney = function (v, m) {
    ...
    if (!m && MODx.lang.ecc_money_unit) {
        m = MODx.lang.ecc_money_unit;
    }
    else if (!m && !MODx.lang.ecc_money_unit) {
        m = '';
    }
    return whole + sub + ' ' + m;
};

ecc.utils.defaultContext = function () {
    var context = MODx.config.default_context;
    return String.format('{0}', context);
};

ecc.utils.userLink = function (value, id) {
    if (!value) {
        return '';
    }
    else if (!id) {
...
ecc.utils.handleChecked = function (checkbox) {
...
ecc.utils.handleColor = function (palette, color) {
...
ecc.utils.renderColor = function (value, props, row) {
    return String.format('<span class="ecc-grid-color" style="background: #{0}"></span>', value);
};
